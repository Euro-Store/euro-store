...
10. AUTH GATE — لا تعيق، بل وجّه
    الزائر يتصفح بحرية كاملة دون قيود — فقط عند إجراء يتطلب حساباً
    (سلة/مفضلة/checkout) يُحوَّل لتسجيل الدخول مع حفظ returnUrl
    للعودة الفورية — لا نقاطع التجربة، نوجّه المستخدم بسلاسة
    Euro Store: يتبع نفس نهج Zalando تماماً في هذا المبدأ
```
...
├─────────────────────────────────────────────────────────────┤
│  3. نظام الاستبدال بـ QR — مساران                           │
│     مسار أ: الزبون يأتي للمحل → الهيلبر (موظف Euro Store)  │
│             يمسح QR → يفحص → يسلّم البديل فوراً (لا شحن)   │
│     مسار ب: الزبون يذهب لمحل شريك مستقل (Partner)          │
│             الشريك يمسح QR → يؤكد → سائق يوصل البديل        │
│     ← يناسب السياق المحلي: بدون شحن عكسي في مسار أ          │
│     ← شبكة محلات شريكة تُوسّع نطاق التغطية الجغرافية       │
├─────────────────────────────────────────────────────────────┤
│  4. دور الهيلبر — Helper Role                               │
│     ← موظف المحل يُدير المخزون ويقترح عبر بوابة مبسطة      │
│     ← اقتراحاته تحتاج موافقة الأدمن ← يمنع الأخطاء         │
│     ← يستقبل استبدال in-store ويسلّم البديل فوراً           │
│     ← يسجّل نقاط الولاء بـ QR الزبون                        │
├─────────────────────────────────────────────────────────────┤
│  5. دور الشريك — Partner Role (نقطة الاستلام)               │
│     ← محل مستقل — لا يرى مخزوناً ولا يقترح شيئاً           │
│     ← فقط: مسح QR + تأكيد/رفض استلام + سجل اليوم           │
│     ← بوابة بسيطة جداً (3 صفحات) — سهلة الاستخدام          │
├─────────────────────────────────────────────────────────────┤
│  6. أكواد الخصم المتقدمة                                    │
│     ← أنواع متعددة: أول استخدام / فوق مبلغ / قسم معين      │
│     ← قابلة للجمع مع نقاط الولاء                           │
│     ← الأدمن يتحكم بكل شيء في الوقت الفعلي                 │
├─────────────────────────────────────────────────────────────┤
│  7. طرق دفع محلية: كاش عند الاستلام + شام كاش              │
