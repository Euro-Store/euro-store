...
interface CartItem { id: string; name: string; brand: string; price: number; image: string; quantity: number; size?: string }
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between mt-3">
          <div className="flex items-center border border-light-border dark:border-dark-border rounded-btn overflow-hidden">
            ...
            <span className="w-8 text-center text-sm font-bold text-light-text dark:text-dark-text">{item.quantity}</span>
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm font-black text-light-text dark:text-dark-text">{f(item.price * item.quantity)}</span>
            <button onClick={() => removeItem(item.id)} className="text-light-muted hover:text-red-500 transition-colors">
              <Trash2 size={15} />
            </button>
          </div>
        </div>
